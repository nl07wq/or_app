...
When the formal workType is holiday, the complete human-readable WORK presentation must be exactly 公休日. Do not continue or qualify it with prose about work, actual work, working hours, breaks, or duration. Do not expose 実働, 勤務時間, 0時間, 0:00, or 休憩0分. Preserve every formal zero value unchanged in source JSON.
...
There is no standalone recovery status or recovery state fact in the formal source schema. Never invent such a concept or describe it as missing, unrecorded, or unavailable in domainEvaluations, crossAnalysis, executionEvaluation, or nextDayHandoff. Evaluate recovery only from formal recovery facts actually present in DAILY AGGREGATE and RECENT CONTEXT, including sleepDurationMinutes, sleepScore, sleepType, and their supplied Recent Context metrics. When sleep duration or sleep score is present, do not generalize recovery as unrecorded. When those sleep facts are null, do not turn the absence of a standalone recovery field into a key factor; use null or [] unless the absence of a specific sleep fact directly prevents an important required evaluation, and then refer only to that specific fact in natural Japanese.
For recovery, evaluate sleep duration against the appropriate 7:00–9:00 range before using RECENT CONTEXT. Both duration below that range and duration above that range can reduce the duration component; a duration that exceeds a recent average is not positive when it is already above the appropriate range. Evaluate sleepScore as a separate quality signal and explain any disagreement without medical diagnosis.
domainEvaluations.nutrition has required presence-based coverage. When each formal current-day value is present in DAILY AGGREGATE, the Nutrition evaluation must include and evaluate intakeCaloriesKcal, proteinG, estimatedExpenditureKcal, and estimatedCalorieBalanceKcal; when all four are present, it must cover all four. Use the corresponding RECENT CONTEXT metric and supplied comparisonLevel when available under the existing Recent Context contract. Do not omit estimated expenditure or estimated calorie balance merely because the same fact also supports crossAnalysis.keyFactors or a cross-domain interaction. This deliberate reuse is not prohibited duplication because the Nutrition domain evaluates the domain itself while Cross Analysis evaluates its wider relationship. When any of these values is null or absent, omit only that metric from the Nutrition evaluation; never convert it to zero, derive it from another value, recalculate calorie balance from intake and expenditure, or estimate expenditure from steps, work, training, or Recent Context.
...
${_plantarRiskCalibrationContract()}
...
Prioritize $currentFact. Use RECENT CONTEXT only to interpret the current fact in its recent direction and range. Each metric contains average, start, end, validCount, and an optional comparisonLevel derived mechanically by Operation Reboot. Numeric zero is a valid fact; null is missing and must never become zero. When comparisonLevel is provided, use that classification exactly for the current-value-versus-average relationship and do not create another threshold or independently strengthen or weaken it. comparisonLevel is separate from trend: do not infer a trend from that classification alone. Do not invent UP, DOWN, or STABLE labels. Do not mechanically dismiss a current value because it is a single-day value. Describe a direction only when the supplied context supports it, avoid strong trend claims when validCount is small, and never infer an unrecorded cause.
RECENT CONTEXT and recentContext are internal schema terms and must never appear in Japanese user-facing prose as 直近文脈, 文脈の平均, 文脈から, context, recent context, callback, or JSON. The supplied period is D-7 through D-1, so describe its average as 直近1週間の平均 or, when repetition would be unnatural, 直近平均. Describe supported direction as 最近の傾向 or 直近の傾向. For comparisonLevel, use natural phrases such as 直近平均を上回っている, 直近平均を下回っている, 直近平均と同程度, or 最近の水準から大きく外れていない without exposing the internal classification name.
For plantarFasciitisLevel, a higher level means a stronger symptom and a lower level means a lighter symptom. In every user-facing Japanese sentence, identify the numeric value as a level, for example 足底筋膜炎LV.4, 足底筋膜炎LVは4だった, or 足底筋膜炎はLV.4だった. Never render it as a bare phrase such as 足底筋膜炎4 or 足底筋膜炎は4. For activity, use only officialSteps. Never mention, compare, or infer measured steps, raw steps, device steps, or the reason for a difference. Expenditure and calorie balance values are stored facts; never recalculate them from other metrics.
''';

  String _plantarRiskCalibrationContract() => '''
DAILY BRIEF PLANTAR RISK CALIBRATION V2
FOOT PAIN LEVEL is the raw morning observation before medication or daytime adaptation. Preserve that number exactly in conditionDisplay.primaryText and never rewrite, normalize, discount, or calculate a different symptom level. Medication context is interpretation only: do not apply a deterministic subtraction or another mathematical correction.
Do not map FOOT PAIN LEVEL directly to operationStatus or an action priority. Evaluate its operational meaning from the supplied current fact and, only when available, supported recent direction, current work exposure, recovery facts, and supplied recent load context. Never invent a worsening trend, standing/walking exposure, planned Training, lower-body Training, or recent lower-body load. This STATUS SOURCE does not provide a planned Training or lower-body-load fact; their absence must not be filled in with an assumption.
Apply these plantar severity guardrails to DAILY BRIEF interpretation:
- LV0–2 is a mild, normally low operational concern. It must not independently make operationStatus yellow or red, create a plantar HIGH action, or create a dedicated protective action. Use calm observation wording only when it adds value.
- LV3–4 is a watch-level concern. It must not independently create a HIGH action. It may contribute to a composite concern only when supported by another supplied risk, such as a supported worsening direction, meaningful current work exposure, or poor sleep/recovery. Without such evidence, use concise monitoring wording rather than aggressive restriction.
- LV5–6 is an elevated concern. A MEDIUM protective action may be appropriate. A HIGH action is exceptional and requires supported compounding risk, such as worsening direction together with meaningful exposure or poor recovery; do not make it HIGH from the level alone.
- LV7 or above is a HIGH-action candidate. Consider the supplied context, but stronger protective guidance is justified when the source supports it.
Treat sleep/recovery as an independent risk domain. Severe sleep deficit may independently drive operationStatus or a HIGH recovery action, but a mild plantar level must not become a co-equal HIGH action merely because sleep is poor. Conversely, a plantar level may contribute to a composite concern only when the supplied facts support that relationship. The final operationStatus remains one central whole-day decision: mild plantar LV alone must not force yellow.
ACTION ROOT-CAUSE DE-DUPLICATION: Keep actions concise and grouped by their dominant supported cause. When several candidate actions describe the same plantar/recovery concern, retain only the highest-value action at HIGH; merge duplicate wording where practical and downgrade supporting monitoring or load-adjustment actions to MEDIUM or LOW. Keep genuinely independent hazards separate. Do not output multiple HIGH actions that merely restate the same plantar/recovery cause.
In Japanese user-facing prose, use calm language for LV0–2, watch/monitor language for LV3–4, protective load-adjustment language for LV5–6, and stronger protection wording for LV7 or above. Do not expose the internal band names or this calibration identifier.
''';
...
Write natural Japanese and do not expose internal terms such as trainingPerformed=false, officialSteps, Formal Fact, sourceType=records, Operational Impact, 直近文脈, 文脈の平均, 文脈から, callback, JSON, context, or recent context. Refer to the user-facing products as DAILY BRIEF and STATUS. Do not use legacy product names in analysis prose. Do not list irrelevant unknowns or statements that something cannot be confirmed. Use notAssessable, null, or a concise rationale only when missing information directly prevents a required evaluation. Adjustments and WATCH POINTS must be supported by current facts or Recent Context; do not add speculative items.
When the formal workType is holiday, the complete human-readable WORK presentation must be exactly 公休日. situationAnalysis.work and workDisplay.primaryText must be 公休日, and workDisplay.supportingText must be null. Do not continue or qualify it with prose about work, actual work, working hours, breaks, or duration. Do not expose 実働, 勤務時間, 0時間, 0:00, or 休憩0分. Preserve every formal zero value unchanged in source JSON.
''';
...
'''
  ...
  String _purpose(String date) => switch (exchangeType) {
    ...
    ReportSyncExchangeType.trainingAnalysis =>
      'Prepare a TRAINING ANALYSIS REPORT for $date.',
    ReportSyncExchangeType.trainingPlan =>
      'Prepare a TRAINING PLAN proposal for $date.',
  ...
  String get _schemaName => switch (exchangeType) {
    ...
    ReportSyncExchangeType.trainingAnalysis =>
      'Training Analysis Schema Version 1',
    ReportSyncExchangeType.trainingPlan => 'Training Plan Schema Version 1',
  ...
  String _sourceRules() => switch (exchangeType) {
    ...
    ReportSyncExchangeType.trainingAnalysis =>
      'Analyze only the supplied Formal Training facts. Do not modify or infer them.',
    ReportSyncExchangeType.trainingPlan =>
      'Create a proposal only from the supplied Formal Training facts.',
    ReportSyncExchangeType.food =>
  ...
  String _fieldRules(String? confirmationDigest) => switch (exchangeType) {
    ...
    ReportSyncExchangeType.trainingAnalysis =>
      'Return analysis only and preserve targetRecordId, operationDate, sourceDigest, exerciseIdentity, and exerciseName exactly.',
    ReportSyncExchangeType.trainingPlan =>
      'Return a proposal only and preserve operationDate, sourceRecordId, sourceDigest, exerciseIdentity, and exerciseName exactly.',
    ReportSyncExchangeType.food =>
