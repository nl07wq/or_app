...
RESPONSE CONTRACT
Return exactly one JSON object and nothing else. Return JSON only: no Markdown fence, explanation, heading, greeting, comment, preface, or afterword. Do not return multiple JSON values. Use the exact COMPLETE RESPONSE SHAPE with schemaVersion "2.0", recordVersion 1, exchangeType "dailyDebrief", operationDate "$operationDate", confirmationDigest null, and packageDigest null. Preserve the sources object below exactly in field content and values. Do not add unknown fields, remove fields, rename fields, omit required fields, or add schema-external sections. Nullable fields must be explicit null. Required array fields must be present and use [] when empty. Every string and array item must be non-empty after trimming. Use only these outcome values: achieved, partiallyAchieved, notAchieved, notAssessable.
Before returning, internally verify that the JSON is syntactically complete and structurally matches the exact schema: every opening and closing brace matches, every opening and closing bracket matches, every quote is closed, there is no trailing comma, the root is one JSON object, every required field exists, and no unknown field exists. Do not output this check or a checklist. The final response must contain only the verified JSON object.
