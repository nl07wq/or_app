...
class TrainingHistoryRangeSelection {
  const TrainingHistoryRangeSelection({required this.period, this.customRange});

  final TrainingHistoryOverviewPeriod period;
  final DateTimeRange? customRange;
}

/// Local display preference for DATA CENTER Training History only.
///
/// It deliberately follows Body History's lightweight preference format while
/// keeping the two History views independently selectable.
class TrainingHistoryRangePreference {
  TrainingHistoryRangePreference({
    Future<SharedPreferences> Function()? preferencesLoader,
  }) : _preferencesLoader = preferencesLoader ?? SharedPreferences.getInstance;

  static const storageKey = 'or_app.data_center.training_history_range.v1';
  static const _version = 1;

  final Future<SharedPreferences> Function() _preferencesLoader;

  Future<TrainingHistoryRangeSelection> load() async {
    try {
      ...
      final periods = TrainingHistoryOverviewPeriod.values.where(
        (value) => value.name == periodName,
      );
      if (periods.length != 1) return _defaultSelection;
      final period = periods.single;
      if (period != TrainingHistoryOverviewPeriod.custom) {
        return TrainingHistoryRangeSelection(period: period);
      }
      final range = _readCustomRange(decoded);
      return range == null
          ? _defaultSelection
          : TrainingHistoryRangeSelection(period: period, customRange: range);
    } catch (_) {
      return _defaultSelection;
    }
  }
  ...
  Future<void> save(
    TrainingHistoryOverviewPeriod period, {
  ...
  static const _defaultSelection = TrainingHistoryRangeSelection(
