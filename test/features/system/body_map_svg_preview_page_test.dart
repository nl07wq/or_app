...
void main() {
  testWidgets(
    'preview supports SVG selection and fixture controls responsively',
    (tester) async {
      for (final width in [320.0, 390.0, 900.0]) {
        await tester.binding.setSurfaceSize(Size(width, 900));
        await tester.pumpWidget(
          const MaterialApp(home: BodyMapSvgPreviewPage()),
        );
        await tester.pumpAndSettle();
        expect(find.text('BODY MAP SVG PREVIEW'), findsOneWidget);
        expect(
          find.byKey(const ValueKey('svg-body-map-front')),
          findsOneWidget,
        );
        ...
        expect(
          find.byKey(const ValueKey('body-map-recovery-fixture')),
          findsOneWidget,
        );
