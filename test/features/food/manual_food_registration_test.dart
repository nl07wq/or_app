...
void main() {
  testWidgets('SAVE TO DB binds the current manual item to its saved master', (
    tester,
  ) async {
    ...
    final packageUnit = find.byType(DropdownButtonFormField<FoodQuantityUnit?>);
    ...
    await tester.tap(
      find.byKey(const ValueKey('food-entry-recalculate-nutrition')),
    );
    await tester.pump();

    await tester.ensureVisible(find.text('SAVE TO DB'));
